...
test.describe('Authentication Test - Simple', () => {
  test('super admin authentication works', async ({ page, superAdminAuth }) => {
    ...
    const heading = page.getByRole('heading', { name: /super admin|administration/i })
  ...
  test('church admin authentication works', async ({ page, churchAdminAuth }) => {
    ...
    const heading = page.getByRole('heading', { name: /admin dashboard|dashboard/i })
    await expect(heading).toBeVisible({ timeout: 10000 })
